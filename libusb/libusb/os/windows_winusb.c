 ...
 */
static HANDLE windows_opener(struct libusb_device_handle *dev_handle, const char *path, DWORD access)
{
...
static int winusbx_open(int sub_api, struct libusb_device_handle *dev_handle)
{
	...
	for (i = 0; i < USB_MAXINTERFACES; i++) {
		if ((priv->usb_interface[i].path != NULL)
			...
			file_handle = windows_opener(dev_handle, priv->usb_interface[i].path, GENERIC_READ | GENERIC_WRITE);
				...
				for (_index = 0; ; _index++) {
					...
					*dev_interface_path_guid_start = '\0';
					...
					if (strncmp(dev_interface_path, priv->usb_interface[iface].path, strlen(dev_interface_path)) == 0) {
						file_handle = windows_opener(dev_handle, filter_path, GENERIC_READ | GENERIC_WRITE);
						if (file_handle != INVALID_HANDLE_VALUE) {
							if (WinUSBX[sub_api].Initialize(file_handle, &winusb_handle)) {
...
static int hid_open(int sub_api, struct libusb_device_handle *dev_handle)
{
	...
	for (i = 0; i < USB_MAXINTERFACES; i++) {
		if ((priv->usb_interface[i].path != NULL)
			...
			hid_handle = windows_opener(dev_handle, priv->usb_interface[i].path, GENERIC_READ | GENERIC_WRITE);
			/*
			...
			if (hid_handle == INVALID_HANDLE_VALUE) {
				...
				hid_handle = windows_opener(dev_handle, priv->usb_interface[i].path, 0);
				if (hid_handle == INVALID_HANDLE_VALUE) {
